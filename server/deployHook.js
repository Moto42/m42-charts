/*
*This is a quick webhook that will pull the new code from a specified repo branch
*when it recieves a POST from github.
...
function _post(req, res) {
  ...
  const sig = 'sha1=' + crypto.createHmac('sha1', process.env.GIT_SECRET)
    .update(JSON.stringify(req.body)).digest('hex');

  if (
    auth !== sig) res.status(401).send('Not Authorized');
  else {
    if (req.body.ref !== `refs/heads/${process.env.REPO_BRANCH}`)
      res.status(200).send('Thankyou, but our code is on another branch.');
    else {
